...
- **enum จำลอง** — โค้ดต้องกันค่านอกชุด: `type` = OPENING|IN|OUT|ADJUST ·
  ...
  Viewer เพิ่ม 10 ก.ค. 2026 ดูอย่างเดียว มีผลจริงตอนเฟส 2 — ดู DATABASE.md ข้อ 6.13)
...
5. **แผนอัปเดตตามเวอร์ชัน `wms-ics-reference/` (ตกลงกัน 10 ก.ค. 2026)** — โฟลเดอร์นี้คือแอปเวอร์ชันที่
   ...
   **ผูก user id เป็น FK ตามธรรมเนียมฐานใหม่ ไม่ใช่ username แบบ reference**) —
   ...
   - [ ] **เฟส 2 backend บนฐานใหม่ ทีละฟีเจอร์:** SSE `/api/events` (สัญญาณเปล่าให้หน้าเว็บ refetch) →
     rate limit → role Viewer → คิวรอส่งมอบ (migration) → session เดี่ยว (migration) → Web Push (migration)
     — migration ใครมันแยก commit ชุดของใครมัน (schema+migration+data_dictionary) ตามข้อ 2
