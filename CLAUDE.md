...
5. **แผนอัปเดตตามเวอร์ชัน `wms-ics-reference/` (ตกลงกัน 10 ก.ค. 2026)** — โฟลเดอร์นี้คือแอปเวอร์ชันที่
   ...
   **ผูก user id เป็น FK ตามธรรมเนียมฐานใหม่ ไม่ใช่ username แบบ reference**) —
   ...
   - [ ] **เฟส 3 ยกหน้าบ้านทั้งชุด** — ตกลง 11 ก.ค. 2026: หน้าบ้าน**ใช้วิธี copy จาก reference แล้ว
     ...
     - [ ] **3A ฐานร่วม+real-time:** deps (เพิ่ม `html5-qrcode`, `jspdf-autotable` · ถอน `html-to-image`
       ที่ grep แล้วไม่มีใครใช้) · utils ทั้งชุด (api ดัก SESSION_REPLACED / events / push / labels /
       confirm / device / thaiFont) · index.css · App + Navbar (กระดิ่ง SSE) + Login + ResetPassword ·
       Skeleton + WelcomeTips · vite.config `host:true` + `src/.env` (เปิดจากมือถือวง LAN — ดู layout ได้
       แต่กล้อง/SW/push ต้องการ secure context จึงทดสอบเต็มที่ได้เฉพาะ localhost บนคอม)
