...
**สถานะ: กำลังย้ายหลังบ้านจาก schema เดิมไปใช้ database ที่รับมอบมา — เส้น products + auth ย้ายเสร็จแล้ว**
(9 ก.ค. 2026: ใช้ `server/prisma.js` เป็น client กลาง + กติกาแยกทดสอบได้ที่ `server/utils/productRules.js`
และ `server/utils/authRules.js` · audit ย้ายมาฐานใหม่แล้วที่ `server/utils/audit.js` — ใช้ `actor_id` FK)
เส้นที่เหลือ (transactions, dashboard) ยังอ้าง schema เก่า (`identifier.sqlite`) อยู่ —
`server/db.js` logAudit เดิม (actor_username) จึงยังไว้ให้ `transactionController` ใช้จนกว่าเส้นนั้นจะย้ายตาม
...
2. **ย้าย data layer ทีละเส้นทาง** (แบบล่าม รักษาทรง JSON เดิม): products → auth → transactions
   ...
   **ลบผู้ใช้ = soft delete** (is_active=false) → username/email ถูกจองถาวร · **เส้นถัดไป: transactions**
   — จุดเชื่อม 2 เส้นที่ทำแล้ว: `productController` logAudit ใช้ `req.user.id` (actor_id FK) +
   ประทับ `created_by` ลงใบ RECEIVE/transaction · `transactionController` ยังใช้ logAudit เก่า (username) จนกว่าจะย้าย
