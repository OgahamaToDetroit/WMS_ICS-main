...
เฟส 3 หน้าบ้านเสร็จครบทั้ง 3A/3B/3C แล้ว และเฟส 4 ตรวจรวมเสร็จแล้ว (11 ก.ค. 2026) —
แผน wms-ics-reference (ข้อ 6.5) จบครบทุกเฟสแล้ว**
**ตรวจล่าสุด:** `npm test` ผ่าน 112 ตัว (99 unit + 12 integration + 1 helper module) ·
...
2. **ย้าย data layer ทีละเส้นทาง** (แบบล่าม รักษาทรง JSON เดิม): products → auth → transactions
   ...
   **→ `server/db.js`/`identifier.sqlite` ไม่มีใครเรียกใช้แล้วทั้ง repo และลบออกแล้ว (10 ก.ค. 2026)**
   ...
   หน้าบ้าน + regression กัน SKU ซ้ำ + **integration tests ที่ `server/test/integration/`
   ดูข้อ 6 ก่อนแตะ** — `npm test` ผ่าน 112 ตัว)
...
5. **แผนอัปเดตตามเวอร์ชัน `wms-ics-reference/` (ตกลงกัน 10 ก.ค. 2026)** — โฟลเดอร์นี้คือแอปเวอร์ชันที่
   ...
   **ผูก user id เป็น FK ตามธรรมเนียมฐานใหม่ ไม่ใช่ username แบบ reference**) —
   ...
   - [x] ✅ **เฟส 3 ยกหน้าบ้านทั้งชุด (เสร็จ 11 ก.ค. 2026)** — หน้าบ้าน**ใช้วิธี copy จาก reference แล้ว
     ...
     - [x] ✅ **3B หน้าหลัก 4 หน้า+สแกนเนอร์ (เสร็จ 11 ก.ค. 2026, 8 commits):** เตรียม server 4 ก้อนก่อน
       ...
       เห็นทุกใบตาม reference บันทึกเหตุผลไว้ DATABASE.md ข้อ 6.17 · S2 เพิ่ม `groupId`/`groupName`
       ...
       (F1 BarcodeScanner+Inventory · F2 Products audit หนักสุด 7 จุด (minStock ทุกจุดว่าง=null รวม
       กับดักตัวที่ 7 ที่ `submitProduct` เดิมใช้ `|| 0`, ตัดช่อง SKU พิมพ์เอง, ล็อกฟอร์ม inbound,
       ตัดปุ่มนำเข้า CSV/ลบถาวร, badge/filter อิง `item.status` แทนเทียบ stock ดิบ, `canArchive`
       เหลือ Admin เท่านั้น) · F3 Homepage (คิวรอส่งมอบ+PDF ไทย copy ตรงตัว เพราะจุดเชื่อมตรง
       ล่ามที่เตรียมจาก S1/S2 พอดี) · F4 UserManagement (แก้ถ้อยคำ confirm ลบให้ตรงความจริง
       soft delete)) ยืนยันทุกก้อนด้วยการคลิกจริงใน browser (สร้าง/แก้ไข/รับเข้า/archive-restore/
       อนุมัติ/ปฏิเสธ/ส่งมอบ/export PDF/ส่งใบเบิกจากตะกร้า) — **ข้อค้นพบส่งต่อ 3C:**
       (1) ⚠️ **เครื่องมือ browser (`computer`/`read_page`) ในสภาพแวดล้อมนี้ไม่เสถียร** — screenshot
       timeout บ่อย, DOM tree ที่ได้จาก `read_page` บางครั้งไม่สะท้อนสถานะล่าสุดหลังคลิก (โดยเฉพาะ modal/
       floating button ที่ render ทีหลัง) วิธีแก้ที่ได้ผล: ใช้ `javascript_tool` ยิง
       `dispatchEvent(new MouseEvent('click',{bubbles:true}))` + `Object.getOwnPropertyDescriptor(...).set`
       สำหรับ input ที่ควบคุมโดย React แทนการคลิก/พิมพ์ผ่าน `computer` โดยตรง แล้วอ่านผลด้วย
       `document.body.innerText`/`fetch` ใน exec เดียวกัน (แยก exec คนละก้อนบางทีจับสถานะไม่ทัน) —
       3C ที่ต้องทดสอบปุ่ม "เปิดแจ้งเตือน"/install prompt ควรเตรียมใช้วิธีนี้เป็นแผนสำรอง
       (2) **`?view=active` ที่ Navbar ใช้มาตั้งแต่ 3A ทำงานถูกต้องทันทีหลัง S1** — payload จาก 8 ใบ
       เหลือ 1 ใบที่ยังค้างจริง ไม่ต้องแตะหน้าบ้านเลย (พิสูจน์แล้วด้วย Network tab)
       (3) ~~ตัวกรอง "เฉพาะสต็อกต่ำ" ใน Products ต้องมีคำค้นด้วยถึงจะเจอของนอก 500 ตัวแรก~~
       **แก้แล้ว (11 ก.ค. 2026):** ย้ายการกรองไปฝั่ง server — `getProducts` รับ `?lowStock=true`
       ใช้ `listLowStockIds` (กติกาเดียวกับเลขการ์ด dashboard — `countLowStock` = `.length` ของมัน)
       สแกนเฉพาะตัวที่ตั้ง `min_stock` แล้ว + groupBy กรองผ่าน relation กัน P2029 ·
       ข้อจำกัด `limit=500` ของ*รายการปกติ*ยังอยู่ (เลื่อนดูของนอก 500 ตัวแรกยังต้องพิมพ์คำค้น)
       (4) ฐานพัฒนามีข้อมูลทดสอบจากเซสชันนี้ตกค้างอยู่ (สินค้า `01262` "ทดสอบสินค้า F2" ตั้ง
       minStock=10 ไว้ทดสอบตัวกรอง, ใบเบิกทดสอบหลายใบ) — ไม่กระทบอะไร ลบทิ้งหรือปล่อยไว้ก็ได้
       (5) สิทธิ์ archive/restore สินค้าเปลี่ยนจาก reference (Admin+Manager) เหลือ **Admin เท่านั้น**
       ตามของเดิม — ถ้า 3C หรือเฟสหลังจะ copy หน้าไหนที่มีปุ่มเกี่ยวกับสินค้าเพิ่ม ให้เช็ค role gate
       ตรงนี้ด้วยเสมอ อย่าเผลอใช้เกณฑ์ของ reference ตรงๆ
     - [x] ✅ **3C ชุด PWA+Push (เสร็จ 11 ก.ค. 2026):** copy `public/` ทั้งชุดจาก reference (icon-192/512
       /maskable-512/apple-touch-icon.png ตัวจริง + `manifest.webmanifest` + `sw.js` — cache ข้าม `/api/`
       ทั้งหมดตามที่ตั้งใจ) · `index.html` เปลี่ยน `lang="th"` + meta PWA (theme-color/apple-mobile-web-app-*)
       · `main.jsx` ลงทะเบียน SW หลัง `window.load` · `InstallPrompt` copy ตรงตัว (banner `beforeinstallprompt`,
       iOS ไม่มี event นี้ตามที่คอมเมนต์ในโค้ดบอกไว้ ถือว่าถูกแล้ว) · `App.jsx` ต่อสาย `<InstallPrompt />`
       ตรงจุดที่ 3B เตรียม comment ไว้ให้แล้ว · commit 2 = `Settings` copy ทั้งไฟล์จาก reference (เดิมเป็น
       สไตล์เก่า/label อังกฤษ ยังไม่เคย migrate ตอน 3B เพราะไม่ใช่ 1 ใน 4 หน้าหลัก — รอบนี้ได้ glass-panel +
       label ไทย + ส่วน "🔔 การแจ้งเตือน" มาพร้อมกัน) — ยืนยันด้วย browser จริงทั้งคู่: manifest fetch 200 +
       ทุก field ตรง, SW register+activate สำเร็จ (`serviceWorker.getRegistrations()` เห็น active:true),
       ไอคอนทั้ง 4 ไฟล์ fetch 200, หน้า Settings render ส่วนแจ้งเตือนถูกกิ่ง (`Notification.permission`
       จริงในเบราว์เซอร์ทดสอบเป็น `denied` → เห็นข้อความบล็อกตามที่โค้ดควรทำ ไม่ใช่บั๊ก) · `npm test`
       ฝั่ง server ผ่านครบ 89 ตัว ณ ตอนจบเฟสนี้ (เซสชันนี้ไม่แตะ server เลย) · ไม่มี util ใหม่ที่เป็น JS ล้วน
       ต้องเขียนเทสต์เพิ่ม (InstallPrompt/Settings เป็น UI component ล้วน ตรวจด้วย browser พอ)
   - [x] ✅ **เฟส 4 ตรวจรวม (เสร็จ 11 ก.ค. 2026):** `npm test` ผ่านครบ 89 ตัว ณ ตอนตรวจเฟส 4 (ไม่เพิ่ม/ไม่ลด — เซสชันนี้
     ไม่แตะโค้ด) · เจาะลึก mutation flow แบบ end-to-end แทนคลิกทุกหน้าซ้ำกับที่ 3B ทำแล้ว (คำแนะนำ
     ที่ปรึกษา: เฟส 4 คือพิสูจน์ว่า "ชิ้นส่วนทำงานร่วมกัน" ไม่ใช่ re-render แต่ละหน้าซ้ำ):
     สมัครสมาชิกใหม่ (`testphase4`) → role เริ่ม Viewer ตามที่ตั้งใจ → admin อนุมัติ+เลื่อนเป็น Operator →
     login คนละ tab → เพิ่มสินค้าลงตะกร้า+ส่งใบเบิก (createOutboundRequest role gate ผ่าน) → **SSE
     cross-tab ยืนยันจริง: ใบใหม่โผล่ในคิว "รออนุมัติ" ของ tab admin ทันทีโดยไม่ reload** → ปฏิเสธทั้งใบ
     (resolveOutcome REJECT branch ถูก: resolved_by≠requested_by → แสดงป้าย "ปฏิเสธ" ตรงตาม §4) → กด
     "รับของแล้ว" ปิดคิวส่งมอบใบเก่าที่ค้างจากเซสชันก่อน (canMarkPickedUp/markPickedUp ทำงานถูก) ·
     ระหว่างทางพิสูจน์ **"session เดี่ยว" ทำงานจริงโดยไม่ได้ตั้งใจ**: login admin ซ้ำจาก tab ที่สองเตะ
     tab แรกกลับหน้า login ทันทีที่ยิง action ถัดไป (SESSION_REPLACED ตามที่ออกแบบไว้ §6.15) · Products/
     Settings โหลด+ตรวจแบบเร็ว (render-only ผ่าน ไม่ซ้ำคลิกทุกปุ่มที่ 3B ทำแล้ว) · **Push: ตรวจได้แค่ UI —
     `Notification.permission='denied'` ในเบราว์เซอร์ทดสอบนี้เหมือนที่ 3C เจอ (ข้อจำกัด sandbox ไม่ใช่บั๊ก)
     Service Worker + PushManager API พร้อมใช้งาน แต่ delivery จริงพิสูจน์ในสภาพแวดล้อมนี้ไม่ได้** ·
     server อยู่บน dev server ที่ session อื่นเปิดค้างไว้ (ไม่ใช่ของเซสชันนี้) จึงไม่มี `preview_logs` ให้ดู —
     ใช้ browser-side evidence (DOM/network/localStorage) แทนตลอด
   (ส่วน "เสิร์ฟ dist/ จาก Express + tunnel" ที่ reference มี = เรื่อง deploy → พักไว้ตามข้อ 0)
6. **แผนปรับปรุงตามผลวิเคราะห์ภายนอก (เริ่ม 12 ก.ค. 2026) — เสร็จ 3 ก้อนแรกแล้ว:**
   - [x] ✅ แก้ช่องโหว่ SKU ซ้ำเบิกเกินสต็อก — `aggregateOutboundItems` รวมบรรทัดก่อนตรวจ
     (`a915da2`) · lint ignore `server/generated/**` + เก็บกวาด README (`c0be820`)
   - [x] ✅ **integration tests 12 flow (12 ก.ค. 2026):** แยก `server/app.js` ออกจาก listen ก่อน
     (`c3fd0f3` — index.js เหลือแค่ listen) แล้วเพิ่ม `supertest` (devDep ฝั่ง server ตัวเดียว
     ไม่เพิ่ม framework) + `server/test/integration/` 3 ไฟล์ (`52158f2`) — auth 4 / products 3 /
     transactions 5 · `setup.js` สร้างฐาน SQLite ชั่วคราวใน os.tmpdir **แยกต่อไฟล์เทสต์**
     (node --test รันคนละ process ห้ามแชร์ฐานข้ามไฟล์) → migrate deploy → fixtures ผ่าน
     Prisma ตรง (ไม่มี seed script ถาวร — ตั้งใจ)
     **กับดักที่เจอจริง — เซสชันหน้าอ่านก่อนแตะเทสต์พวกนี้:**
     - **Node 25 บล็อก spawn ไฟล์ `.cmd` ที่ไม่มี `shell:true` → EINVAL** (security fix
       CVE-2024-27980) และ npx ใน shell non-interactive อาจแขวนรอ prompt → เรียก Prisma CLI
       ตรงด้วย `spawnSync(process.execPath, [<node_modules/prisma/build/index.js>, ...])`
     - schema engine ไม่สร้างไฟล์ฐานใหม่ใน temp เอง → pre-create ไฟล์ว่างก่อน migrate deploy
     - **ต้องตั้ง `DATABASE_URL` ก่อน dynamic import `app.js`/`prisma.js`** — dotenv ไม่ทับ env
       ที่ตั้งแล้ว แต่ static import จะ hoist ไปโหลดก่อนตั้งค่า = เทสต์เขียนใส่ฐานพัฒนาเงียบๆ
       (เกณฑ์กันพลาด: mtime/size ของ `warehouse.dev.db` ก่อน/หลัง `npm test` ต้องเท่ากัน)
     - **ห้ามยิง `GET /api/events` ผ่าน supertest** — SSE เปิด connection ค้าง เทสต์แขวนไม่จบ
       (พฤติกรรม SSE พิสูจน์ด้วย browser ไปแล้วตอนเฟส 3/4)
     - `node --test` นับ `setup.js` (ไฟล์ใต้ `test/` ที่ไม่มี test case) เป็น 1 pass
       → 112 = 99 unit + 12 integration + 1 helper
   - [ ] งานที่เหลือจากแผน: เคาะเรื่องจำนวนทศนิยม (schema ตั้งใจให้ `Float` เช่น สายไฟเป็นเมตร
     แต่ controller รับเฉพาะจำนวนเต็ม — **ขัดกันอยู่ รอเจ้าของเคาะ**ว่าคลังมีเบิกเป็นเศษจริงไหม) ·
     ย่อไฟล์นี้ + แยก session log ออกจากกฎถาวร · แยกไฟล์ใหญ่ (Homepage/Products ~530 บรรทัด)
     เมื่อแตะครั้งถัดไป · `npm audit` ฝั่ง server มี moderate 3 รายการ (ยังไม่แก้ — จดไว้ก่อน)
