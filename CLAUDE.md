...
**สถานะ: กำลังย้ายหลังบ้านจาก schema เดิมไปใช้ database ที่รับมอบมา — เส้น products ย้ายเสร็จแล้ว**
(9 ก.ค. 2026: ใช้ `server/prisma.js` เป็น client กลาง + กติกาแยกทดสอบได้ที่ `server/utils/productRules.js`)
เส้นที่เหลือ (auth, transactions, dashboard, audit log) ยังอ้าง schema เก่า (`identifier.sqlite`) อยู่
...
2. **ย้าย data layer ทีละเส้นทาง** (แบบล่าม รักษาทรง JSON เดิม): products → auth → transactions
   ...
   bulk import พักทั้งปุ่ม+endpoint · ฟอร์ม React ถอนกับดัก `|| 10` แล้ว · **เส้นถัดไป: auth**
   — หมายเหตุช่วงเปลี่ยนผ่าน: `created_by` ของใบ/transaction ฝั่งใหม่ยังเป็น NULL โดยตั้งใจ
   (ตาราง `users` ฐานใหม่ยังว่างจนกว่าเส้น auth จะย้ายตาม — ผู้กระทำถูกจดใน audit ฝั่งเก่าแทน)
   ...
   เอา testing "แบบเบา" ไม่ต้องครอบทุก endpoint (ของเส้น products: `server/test/productRules.test.js`)
