...
**`server/db.js` (`identifier.sqlite`) ไม่มีใครเรียกใช้แล้วทั้ง repo** (grep ยืนยันแล้ว) — เหลืองานเดียวคือลบไฟล์ทิ้งจริง (เก็บกวาด)
...
3. **เก็บกวาดฐานเก่า (ถัดไป):** ลบ `server/db.js` + `identifier.sqlite` + ไฟล์ตายที่เกี่ยวข้อง (`data/users.json` ถ้ายังอยู่)
   + อัปเดตเอกสารว่าย้ายครบแล้ว — ไม่มีความเสี่ยงด้าน logic เหลือ (grep ยืนยันไม่มีใครเรียก db.js แล้ว)
