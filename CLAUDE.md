...
**สถานะ: กำลังย้ายหลังบ้านจาก schema เดิมไปใช้ database ที่รับมอบมา** — โค้ด `server/` ปัจจุบัน
(controllers, `db.js`) ยังอ้าง schema เก่า (`identifier.sqlite`) อยู่ จะถูกเขียนใหม่ทีละเส้นทาง
...
| `DATABASE.md` | ระเบียบการจัดการฐานข้อมูล + **การตัดสินใจ 4 ข้อที่ล็อกแล้ว (ข้อ 6)** |
...
1. **Migration แรก:** เติมคอลัมน์ auth ลง `users` (`username` unique, `password_hash` แบบ bcrypt,
   email, status, avatarUrl) + role 3 ค่า — ตอนนี้ `users` มีแค่ id/name/role/is_active
