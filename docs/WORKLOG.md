...
## 12 ก.ค. 2026 — QR × Inventory: แยกเป็น shared hook + เก็บรอยลบพลาด

- **`e69016b`** — ย้าย `useProductQrScan` → `src/hooks/useWarehouseProductScan.js` ใช้ร่วม Products+Inventory
  (logic เดิมทั้งหมด · `lowStockOnly`/`searchParams`/`setSearchParams` เป็น optional default — Products
  ส่งครบเหมือนเดิม พฤติกรรมไม่เปลี่ยน · Inventory ไม่ส่ง → บล็อกลบ URL filter ถูก guard ปิด) ·
  Inventory เดิมสแกนแบบ raw code (QR 9 หลักค้นไม่เจอ) ตอนนี้เข้า parser เส้นเดียวกัน: กล้อง+USB
  (Enter/ไม่ส่ง Enter) → item_id 5 หลักท้าย · ล้าง group filter · แจ้งพบ/ไม่พบ/รหัสผิด · `scanVersion`
  กันแจ้งผลย้อนหลัง · สแกน = กรองอย่างเดียว ไม่แตะตะกร้า/ใบเบิก · ไม่แตะ API/DB
- รีวิวอิสระ verdict = พร้อมให้ผู้ใช้ทดสอบ · ข้อควรระวังรอทดสอบเครื่องจริง (inherit มา 2 หน้าเพราะ hook ร่วม):
  เครื่องสแกนที่ตามด้วย **Tab** จะโดน `onBlur`→`cancelArmedScan` ยกเลิกก่อน timer 180ms ยิง
  (สอง config ที่รองรับ — Enter / ไม่มี suffix — ไม่กระทบ)
- **เก็บรอยลบพลาดใน `qr_spec.md`:** เจ้าของเผลอมือลบข้อความตาราง §5.2 หายครึ่งประโยค
  ("สแกนเพี้ยนจะกลายเป็นขยะใน master ทันที") — `git restore` คืนเป็นฉบับ commit (ไม่ใช่การแก้เนื้อหา)
