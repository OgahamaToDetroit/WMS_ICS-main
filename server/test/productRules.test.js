...
test('mapItemToProduct: ปล่อย NULL ผ่านตามจริง ห้ามเสก 10/0 ทับ', () => {
  ...
  assert.equal(product.groupId, '02');
...
test('mapItemToProduct: ยอดติดลบต้องติดป้าย warning แบบเดียวกับระบบเดิม', () => {
  const product = mapItemToProduct(
    { item_id: '19001', name: 'x', group_id: '19', group: { group_name: 'g' }, min_stock: 5, is_active: true },
    -2
  );
...
test('mapItemToProduct: isActive ปล่อยผ่านตามจริง — false ต้องไม่หายเป็น undefined', () => {
  const product = mapItemToProduct(
    { item_id: '02002', name: 'y', group_id: '02', group: { group_name: 'g' }, min_stock: null, is_active: false },
    3
  );
...
// ---------------------------------------------------------------------------
// กติกา: ตัวกรอง ?lowStock=true ของ getProducts ต้องใช้กติกาเดียวกับเลขบนการ์ด dashboard
// (บั๊กที่แก้ 11 ก.ค. 2026: การ์ดบอกมี 1 แต่กดเข้าไปว่าง เพราะหน้า Products กรองเองจากแค่ 500 ตัวแรก)
// ---------------------------------------------------------------------------

test('listLowStockIds: คืนเฉพาะรหัสที่ Low Stock จริง — NULL ไม่นับ, ของหมดไม่นับ, เท่าเกณฑ์พอดีนับ', () => {
  const items = [
    { item_id: 'A', min_stock: null }, // ยังไม่ตั้งเกณฑ์ → ไม่มีวันติด
    { item_id: 'B', min_stock: 5 },    // stock 3 ≤ 5 → ติด
    { item_id: 'C', min_stock: 10 },   // ไม่มีแถว transaction → stock 0 = Out of Stock ไม่ใช่ Low
    { item_id: 'D', min_stock: 7 }     // stock 7 เท่าเกณฑ์พอดี → ติด (พฤติกรรมเดิมของระบบ)
  ];
  const stockMap = buildStockMap([
    { item_id: 'A', _sum: { qty_change: 1 } },
    { item_id: 'B', _sum: { qty_change: 3 } },
    { item_id: 'D', _sum: { qty_change: 7 } }
  ]);
  assert.deepEqual(listLowStockIds(items, stockMap), ['B', 'D']);
});

test('listLowStockIds กับ countLowStock ต้องให้ผลตรงกันเสมอ (เลขการ์ด = จำนวนรายการที่คลิกเข้าไปเห็น)', () => {
  const items = [{ item_id: 'B', min_stock: 5 }, { item_id: 'E', min_stock: null }];
  const stockMap = buildStockMap([{ item_id: 'B', _sum: { qty_change: 2 } }, { item_id: 'E', _sum: { qty_change: 1 } }]);
  assert.equal(listLowStockIds(items, stockMap).length, countLowStock(items, stockMap));
});
