...
// ---------------------------------------------------------------------------
// กติกา: 1 บัญชี = 1 อุปกรณ์ (DATABASE.md ข้อ 6.15) — sid ใน token ต้องตรง session ล่าสุด
// ---------------------------------------------------------------------------

test('checkSession: sid ตรงกับ session ล่าสุด → OK', () => {
  assert.equal(checkSession('abc123', 'abc123'), 'OK');
});

test('checkSession: token ไม่มี sid (รุ่นก่อนมีฟีเจอร์) → MISSING_SID ใช้ไม่ได้ทันที — เข้มกว่า reference โดยตั้งใจ', () => {
  assert.equal(checkSession(undefined, 'abc123'), 'MISSING_SID');
  assert.equal(checkSession(undefined, null), 'MISSING_SID'); // แม้ database ยังไม่มี session ก็ห้ามผ่าน (reference ยอมให้ผ่านเคสนี้)
  assert.equal(checkSession('', 'abc123'), 'MISSING_SID');
});

test('checkSession: sid ไม่ตรง (มี login ใหม่กว่า) → REPLACED เครื่องเก่าโดนตัด', () => {
  assert.equal(checkSession('old-device', 'new-device'), 'REPLACED');
  assert.equal(checkSession('old-device', null), 'REPLACED'); // token มี sid แต่ฐานไม่มี session (เช่นฐานถูกรีเซ็ต) = token ค้างโลก
});
